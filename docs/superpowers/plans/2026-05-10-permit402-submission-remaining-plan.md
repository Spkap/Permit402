...
| Anchor frontend adapter | apps/web/lib/permit402/adapter.ts and anchor-adapter.ts now exist. The Anchor adapter can read a configured PolicyVault plus matching Receipt/BlockedAttempt accounts from RPC. | UI is still mock-only by default and cannot write localnet/devnet state yet. |
